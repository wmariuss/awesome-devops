...
- [HookRun](https://github.com/bluvenr/hookrun) - Lightweight webhook action engine that executes custom commands and scripts based on YAML rules when webhook requests arrive. Token/HMAC/IP authentication, execution policies (block/always/cooldown), hot reload, single static binary written in Go.
